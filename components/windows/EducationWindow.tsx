...
function getIcon(degree: string) {
  if (degree.includes('DAW'))  return <IconWeb />;
  if (degree.includes('Ciber')) return <IconShield />;
  if (degree.includes('SMR'))  return <IconNetwork />;
  return <IconBook />;
}
    ...
    >
      ...
      <div style={{ flex: '1 1 0%', minHeight: 0, overflowY: 'auto', position: 'relative', zIndex: 1, scrollbarWidth: 'thin', scrollbarColor: isDark ? 'rgba(0,245,255,.2) transparent' : 'rgba(0,0,0,.15) transparent' }}>
        {education.map((entry, idx) => {
          const accent = getAccent(idx, entry.current);
          const C = cardStyles(accent);
          const badgeKey = entry.current ? 'edu.inProgress' : 'edu.finished';
          const descText = entry.degree.includes('Ciber') ? t('edu.desc.ciberseguridad', lang) : entry.description;
          ...
          return (
            <div key={entry.degree}>
              <div style={{
                ...
                {/* Accent glow */}
                <div style={{ position: 'absolute', top: -40, right: -40, width: 160, height: 160, borderRadius: '50%', background: `radial-gradient(circle, ${C.accentBg}, transparent 70%)`, pointerEvents: 'none' }} />

                {/* Header row */}
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, position: 'relative' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                    <div style={{ width: 44, height: 44, borderRadius: 12, flexShrink: 0, background: `${accent}18`, border: `1px solid ${accent}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: accent }}>
                      {getIcon(entry.degree)}
                    </div>
                    <div>
                      ...
                      <div style={{ fontFamily: MONO, fontSize: 10, color: C.subCol, marginTop: 4 }}>{entry.institute}</div>
                      <div style={{ fontFamily: MONO, fontSize: 10.5, color: C.periodCol, marginTop: 3 }}>{entry.period}</div>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 14, paddingLeft: 58 }}>
                    {entry.tags.map(tag => (
                      <span key={tag} style={{
                        ...
                        color: accent, background: `${accent}10`, border: `1px solid ${accent}28`,
                      }}>
                        {tag}
                      </span>
                    ))}
              ...
              {idx < education.length - 1 && (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '6px 0' }}>
                  <div style={{ width: 1, height: 14, background: `linear-gradient(180deg, ${accent}80, ${getAccent(idx + 1, education[idx + 1].current)}80)` }} />
                  ...
                  <div style={{ width: 1, height: 14, background: `linear-gradient(180deg, ${getAccent(idx + 1, education[idx + 1].current)}80, transparent)` }} />
                </div>
              )}
            </div>
          );
